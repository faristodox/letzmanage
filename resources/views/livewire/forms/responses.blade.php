<div class="space-y-6">
    ...
    <!-- Analysis -->
    @if ($numberTotals->isNotEmpty() || $charts->isNotEmpty())
        <div>
            <h2 class="mb-3 text-sm font-semibold text-slate-900">{{ __('Analysis') }}</h2>

            @if ($numberTotals->isNotEmpty())
                <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($numberTotals as $numberTotal)
                        <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                            <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">{{ $numberTotal['field']->label }} ({{ __('Total') }})</p>
                            <p class="mt-1 text-2xl font-bold text-slate-900">{{ number_format($numberTotal['total'], 2) }}</p>
                        </div>
                    @endforeach
                </div>
            @endif
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                        ...
                        <tr wire:key="response-{{ $response->id }}" class="hover:bg-slate-50">
                            ...
                            @foreach ($previewFields as $field)
                                ...
                                <td class="max-w-xs truncate px-4 py-3 text-sm text-slate-700">
                                    @if ($field->type->value === 'file' && $answer)
                                        <a href="{{ \Illuminate\Support\Facades\Storage::url($answer) }}" target="_blank" rel="noopener" class="text-indigo-600 hover:text-indigo-700">{{ __('View file') }}</a>
                                    @else
                    ...
                    @empty
                        <tr>
                            @php $columnCount = 3 + $previewFields->count(); @endphp

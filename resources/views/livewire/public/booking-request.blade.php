                    ...
                    <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        @forelse ($spaces as $space)
                            @if ($space->isUnderMaintenance())
                                {{-- Under maintenance / renovation — not bookable --}}
                                <div class="relative overflow-hidden rounded-2xl border border-amber-200 bg-white text-left shadow-sm cursor-not-allowed">
                                    <div class="relative">
                                        @if ($space->image_path)
                                            <img src="{{ \Illuminate\Support\Facades\Storage::url($space->image_path) }}" alt="{{ $space->name }}" class="h-40 w-full object-cover grayscale">
                                        @else
                                        ...
                                        @endif
                                        <span class="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-amber-500 px-2.5 py-1 text-xs font-semibold text-white shadow">
                                    ...
                                    <div class="p-4">
                                        ...
                                        <div class="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-700">
                                            <p class="font-medium">{{ $space->maintenance_note ?: 'Temporarily unavailable.' }}</p>
                                            @if ($space->maintenance_until)
                                                <p class="mt-0.5 text-xs text-amber-600">Expected to reopen {{ $space->maintenance_until->format('j M Y') }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
